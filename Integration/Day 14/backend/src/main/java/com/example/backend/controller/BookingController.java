...
public class BookingController {
    ...
    public ResponseEntity<Long> getTotalBookingsCount() {
        try {
            long totalBookingsCount = service.countTotalBookings();
            return ResponseEntity.ok(totalBookingsCount);
        } catch (Exception e) {
    ...
    public ResponseEntity<List<Booking>> getBookingsByEmail(@RequestParam String email) {
        List<Booking> bookings = service.getBookingsByEmail(email);
        return new ResponseEntity<>(bookings, HttpStatus.OK);
    }


    @GetMapping("/count")
    public ResponseEntity<Map<String, Long>> countBookingsByBoatType() {
        try {
            Map<String, Long> counts = service.countBookingsByBoatType();
